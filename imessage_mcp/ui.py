"""In-house gradient text renderer over rich (iMessage-blue house style).

Mirrors wa-cli/ui.py: linearly interpolates hex colors across a string and
emits a rich.text.Text with per-character styles. Provides the banner, status
badges, gradient-accented tables, a spinner, an error panel, and the shared
`console`.
"""
...
_BANNER_START = "#1FA2FF"  # iMessage light blue
_BANNER_END = "#0A84FF"  # iMessage system blue
...
def banner() -> Text:
    """Gradient banner for the `imsg` CLI."""
    return gradient_text("imsg · iMessage CLI", _BANNER_START, _BANNER_END)


def badge(state: str) -> Text:
    """Status badge: up, down, degraded, warn, or sent."""
    try:
...
def spinner(msg: str) -> Status:
    """Wrap rich.status.Status for use while an operation is in flight."""
